...
export default function ChatPage() {
  ...
  const handleFileUpload = async (e) => {
    ...
    if (!file || !user) return;
    const filePath = `chats/${user.id}/${Date.now()}`;
  ...
  if (!user) return (
    <div style={{ maxWidth: '400px', margin: '100px auto', padding: '30px', background: '#0a0a0a', color: '#fff', borderRadius: '20px', border: '2px solid #800000', textAlign: 'center' }}>
      ...
      <input type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} style={{ width: '100%', padding: '12px', margin: '10px 0', background: '#1a1a1a', color: '#fff', border: '1px solid #333', outline: 'none' }} />
      <input type="password" placeholder="Pw" value={password} onChange={e => setPassword(e.target.value)} style={{ width: '100%', padding: '12px', marginBottom: '20px', background: '#1a1a1a', color: '#fff', border: '1px solid #333', outline: 'none' }} />
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto', height: '100dvh', display: 'flex', flexDirection: 'column', background: '#000', color: '#fff', position: 'relative' }}>
      ...
      {isModalOpen && (
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ width: '100%', maxWidth: '300px', background: '#1a1a1a', padding: '30px', borderRadius: '25px', border: '2px solid #800000', textAlign: 'center' }}>
            <label style={{ display: 'block', margin: '0 auto 20px', width: '100px', height: '100px', borderRadius: '50%', border: '2px solid #D4AF37', overflow: 'hidden', cursor: 'pointer', background: '#222' }}>
            ...
            <input value={profile.username} onChange={e => setProfile({ ...profile, username: e.target.value })} placeholder="Name" style={{ width: '100%', padding: '10px', background: '#000', color: '#fff', border: '1px solid #333', marginBottom: '20px', textAlign: 'center', outline: 'none' }} />
            <button onClick={saveProfile} style={{ width: '100%', padding: '10px', background: '#800000', color: '#fff', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>SAVE</button>
      ...
      <header style={{ padding: '10px 25px', background: '#800000', display: 'flex', justifyContent: 'space-between', alignItems: 'center', minHeight: '80px', borderBottom: '2px solid #D4AF37', flexShrink: 0 }}>
        <h1 style={{ fontSize: '2.2rem', fontFamily: 'serif', fontStyle: 'italic', letterSpacing: '2px' }}>for VAU</h1>
          ...
          return (
            <div key={m.id} style={{ marginBottom: '20px', textAlign: isMe ? 'right' : 'left' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: isMe ? 'flex-end' : 'flex-start' }}>
                ...
                <span style={{ fontSize: '0.6rem', color: '#ccc', marginTop: '4px' }}>
      ...
      <div style={{ padding: '15px 20px', background: '#800000', display: 'flex', gap: '10px', alignItems: 'flex-end', borderTop: '2px solid #D4AF37', flexShrink: 0 }}>
        <label style={{ background: '#000', width: '38px', height: '38px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
          <CameraIcon />
        ...
        <textarea 
          ...
          placeholder="Message..." 
          style={{ flex: 1, padding: '8px 18px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.3)', outline: 'none', resize: 'none', height: '38px', background: '#800000', color: '#fff', fontSize: '14px', lineHeight: '20px' }} 
        />
        <button onClick={() => handleSend()} style={{ background: '#000', color: '#D4AF37', width: '60px', height: '38px', borderRadius: '20px', border: 'none', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer', flexShrink: 0 }}>SEND</button>
      </div>
    </div>
  );
}
