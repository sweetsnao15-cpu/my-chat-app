...
export default function GuestPage() {
  ...
  return (
    <div style={{ width: '100%', height: '100dvh', display: 'flex', flexDirection: 'column', background: '#000', color: '#fff', overflow: 'hidden', fontFamily: 'serif' }}>
      ...
      {/* ヘッダー：赤色をほんの少し濃く (#4a0000) */}
      <header style={{ 
        padding: '35px 15px', background: '#4a0000', borderBottom: '1px solid #D4AF37', 
        display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', flexShrink: 0, zIndex: 10 
      }}>
        <h1 style={{ fontSize: '1.8rem', fontStyle: 'italic', fontWeight: 'bold', margin: 0, letterSpacing: '5px', color: '#fff' }}>
          for VAU
        </h1>
      ...
      {isMenuOpen && (
        <div style={{ 
          ...
          <div style={{ width: '80%', maxWidth: '300px', textAlign: 'center' }}>
            ...
            {!user ? (
              <button onClick={handleSignIn} style={{ width: '100%', padding: '15px', background: '#D4AF37', color: '#000', border: 'none', borderRadius: '30px', fontWeight: 'bold', marginBottom: '20px', cursor: 'pointer' }}>Googleでログイン</button>
            ) : (
              <>
                <div style={{ marginBottom: '30px', color: '#D4AF37', fontSize: '0.9rem' }}>Logged in as: <br/><span style={{ color: '#fff' }}>{user.email}</span></div>
                <button onClick={handleSignOut} style={{ width: '100%', padding: '15px', background: 'transparent', color: '#ff4d4d', border: '1px solid #ff4d4d', borderRadius: '30px', fontWeight: 'bold', marginBottom: '20px', cursor: 'pointer' }}>LOGOUT</button>
              </>
            )}
            <button onClick={() => setIsMenuOpen(false)} style={{ width: '100%', padding: '15px', background: 'transparent', color: '#fff', border: '1px solid #333', borderRadius: '30px', cursor: 'pointer' }}>CLOSE</button>
          </div>
        </div>
      )}
      ...
      {/* フッター：赤色をほんの少し濃く (#4a0000) */}
      <footer style={{ 
        padding: '12px 15px', background: '#4a0000', borderTop: '1px solid #D4AF37', 
        ...
        <div style={{ maxWidth: '600px', margin: '0 auto', display: 'flex', gap: '12px', alignItems: 'flex-end' }}>
          ...
          <button
            ...
            style={{ 
              ...
              borderRadius: '22px', padding: '10px 20px', fontWeight: 'bold', fontSize: '0.85rem', cursor: 'pointer', height: '42px'
            }}
          >
            SEND
          </button>
