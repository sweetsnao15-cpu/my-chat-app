...
export default function ChatPage() {
  ...
  const loadM = async (u, ad, sc) => {
    ...
    setMs(d.filter(m => isAdm || m.user_id === u.id));
  ...
  const send = async (img = null) => {
    ...
    const t = ct; setCt('');
    const { error: e } = await supabase.from('messages').insert([{ content: img || t, user_id: user.id, is_image: !!img, receiver_id: H }]);
    ...
    <div style={{ 
      ...
      <header style={{ 
        ...
        <h1 style={{ 
          ...
          letterSpacing: '3px', 
          margin: 0,
          paddingLeft: '25px', // 文字をさらに右に寄せました
        ...
        <div onClick={() => setSetOk(true)} style={{ cursor: 'pointer' }}>
          {pr.avatar_url ? 
            <img src={pr.avatar_url} style={{ width: '48px', height: '48px', borderRadius: '50%', objectFit: 'cover', border: '2px solid #D4AF37' }} /> 
            : 
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ width: '100%', maxWidth: '300px', background: '#1a1a1a', padding: '30px', borderRadius: '25px', border: '2px solid #800000', textAlign: 'center' }}>
            <label style={{ display: 'block', margin: '0 auto 20px', width: '100px', height: '100px', borderRadius: '50%', border: '2px solid #D4AF37', overflow: 'hidden', cursor: 'pointer', position: 'relative' }}>
              {pr.avatar_url ? <img src={pr.avatar_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <InitialAvatar name={pr.username} size="100px" fontSize="3rem" />}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
        {ms.map(m => (
          <div key={m.id} style={{ marginBottom: '20px', textAlign: 'right' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '4px' }}>
              <div 
                ...
                style={{ 
                  ...
                  background: '#800000', borderRadius: '18px 18px 0 18px', color: '#fff', 
                ...
                {m.is_image ? <img src={m.content} style={{ maxWidth: '100%', borderRadius: '12px', display: 'block', pointerEvents: 'none' }} /> : m.content}
              </div>
      ...
      <div style={{ padding: '15px 20px', background: '#800000', display: 'flex', gap: '10px', alignItems: 'flex-end', borderTop: '2px solid #D4AF37', zIndex: 10 }}>
        ...
        <textarea ref={txRef} value={ct} onChange={e => setCt(e.target.value)} placeholder="Message..." style={{ flex: 1, padding: '8px 18px', borderRadius: '20px', border: 'none', outline: 'none', resize: 'none', height: '38px', fontSize: '16px', lineHeight: '22px', userSelect: 'text', WebkitUserSelect: 'text' }} />
        <button onClick={() => send()} style={{ background: '#000', color: '#fff', width: '60px', height: '38px', borderRadius: '20px', border: 'none' }}>SEND</button>
      </div>
    </div>
  );
}