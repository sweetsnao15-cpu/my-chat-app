  ...
  if (!user) return (
    <div style={{ minHeight: '100dvh', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ width: '100%', maxWidth: '400px', padding: '40px 30px', background: '#0a0a0a', color: '#fff', borderRadius: '30px', border: '2px solid #800000', textAlign: 'center' }}>
        <h2 style={{ color: '#800000', fontSize: '2.5rem', fontFamily: 'serif', fontStyle: 'italic', marginBottom: '30px' }}>{isSignUp ? "SIGN UP" : "for VAU"}</h2>
        ...
        <p onClick={() => setIsSignUp(!isSignUp)} style={{ marginTop: '25px', fontSize: '0.9rem', color: '#666', cursor: 'pointer' }}>{isSignUp ? "Already have an account? Login" : "New here? Sign Up"}</p>
      </div>
    </div>
  );
          ...
          return (
            <div key={m.id} style={{ marginBottom: '22px', textAlign: isMe ? 'right' : 'left' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: isMe ? 'flex-end' : 'flex-start' }}>
                <div style={{ padding: m.is_image ? '5px' : '12px 18px', background: isMe ? '#800000' : '#2a2a2a', borderRadius: isMe ? '20px 20px 0 20px' : '20px 20px 20px 0', maxWidth: '85%', color: '#fff', border: isMe ? 'none' : '1px solid #333', wordBreak: 'break-all' }}>
