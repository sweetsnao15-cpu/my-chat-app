...
export default function ChatPage() {
  ...
  const [contextMenu, setContextMenu] = useState(null); // メニュー用
  ...
  const handleSend = async (imgUrl = null) => {
    ...
    await supabase.from('messages').insert([{
      content: contentBody, user_id: user.id, is_image: !!imgUrl, receiver_id: ADMIN_ID
    }]);
  };
  ...
  if (!user) return (
    ...
    <div style={{ minHeight: '100dvh', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ width: '100%', maxWidth: '400px', padding: '40px 30px', background: '#0a0a0a', color: '#fff', borderRadius: '30px', border: '2px solid #800000', textAlign: 'center' }}>
        ...
        }} style={{ width: '100%', padding: '15px', background: '#800000', color: '#fff', fontWeight: 'bold', border: 'none', borderRadius: '10px' }}>{isSignUp ? "CREATE ACCOUNT" : "LOG IN"}</button>
      </div>
    </div>
  );
  ...
  return (
    <div onClick={() => setContextMenu(null)} style={{ width: '100%', height: '100dvh', display: 'flex', flexDirection: 'column', background: '#000', color: '#fff', position: 'relative' }}>
      ...
      {/* 操作メニュー */}
      {contextMenu && (
        <div style={{ position: 'fixed', top: contextMenu.y, left: contextMenu.x, background: '#1a1a1a', border: '1px solid #D4AF37', borderRadius: '8px', zIndex: 1000, overflow: 'hidden' }}>
          <div onClick={() => copyToClipboard(contextMenu.message.content)} style={{ padding: '10px 20px', cursor: 'pointer', borderBottom: '1px solid #333' }}>コピー</div>
      ...
      {isModalOpen && (
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.95)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ width: '100%', maxWidth: '320px', background: '#1a1a1a', padding: '30px', borderRadius: '25px', border: '2px solid #800000', textAlign: 'center' }}>
            ...
            <button onClick={async() => await supabase.auth.signOut()} style={{ width: '100%', marginTop: '10px', background: 'transparent', color: '#666', border: 'none' }}>LOGOUT</button>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
        {messages.map((m, i) => {
          ...
          const showDate = mDate !== prevDate;

          return (
            <div key={m.id}>
              {showDate && (
                <div style={{ textAlign: 'center', margin: '30px 0', fontSize: '0.8rem', color: '#666', fontFamily: 'serif' }}>
                  ― {mDate} ―
                </div>
              )}
      ...
      <div style={{ padding: '15px 20px', background: '#800000', display: 'flex', gap: '12px', alignItems: 'flex-end', borderTop: '2px solid #D4AF37', paddingBottom: 'calc(15px + env(safe-area-inset-bottom))' }}>
        ...
        <textarea ref={textareaRef} value={inputText} onChange={e => setInputText(e.target.value)} placeholder="Message..." 
          style={{ flex: 1, padding: '10px 20px', borderRadius: '22px', border: '1px solid rgba(255,255,255,0.3)', outline: 'none', resize: 'none', height: '42px', minHeight: '42px', background: '#800000', color: '#fff', fontSize: '16px', lineHeight: '22px' }} 
        />

        <button onClick={() => handleSend()} style={{ background: '#000', color: '#D4AF37', width: '75px', height: '42px', borderRadius: '22px', border: 'none', fontSize: '16px', fontWeight: 'bold', cursor: 'pointer', fontFamily: 'serif', fontStyle: 'italic', marginBottom: '2px' }}>SEND</button>
      </div>
    </div>
  );
}
