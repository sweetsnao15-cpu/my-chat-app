...
function AdminComponent() {
  ...
  const renderMessages = () => {
    ...
    return (
      <div style={{ maxWidth: '340px', margin: '0 auto', width: '100%', paddingBottom: '20px' }}>
            ...
            <div key={m.id}>
              {isNewDay && (
                <div style={{ display: 'flex', justifyContent: 'center', margin: '20px 0' }}>
                  <div style={{ color: '#D4AF37', fontSize: '0.6rem', letterSpacing: '2px', fontWeight: 'bold' }}>{dateStr}</div>
                </div>
              )}
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', flexDirection: isMe ? 'row-reverse' : 'row', width: '100%' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: isMe ? 'flex-end' : 'flex-start', maxWidth: '90%' }}>
                    {!isMe && viewMode === 'GLOBAL' && (
                      <span style={{ fontSize: '0.75rem', color: '#D4AF37', fontWeight: 'bold', marginBottom: '4px' }}>{senderProfile?.username || 'Guest'}</span>
                    )}
                    <div style={{ display: 'flex', alignItems: 'flex-end', gap: '4px', flexDirection: isMe ? 'row-reverse' : 'row' }}>
                      <div 
                        onContextMenu={(e) => { e.preventDefault(); e.stopPropagation(); setActiveMenuId(m.id); }}
                        style={{ 
                          ...
                          fontSize: '0.85rem', color: '#fff', whiteSpace: 'pre-wrap', wordBreak: 'break-word', position: 'relative'
                        }}>
                        {m.is_image ? <img src={m.content} onLoad={scrollToBottomInstant} style={{ maxWidth: '100%', borderRadius: '10px', display: 'block' }} /> : m.content}
                        {activeMenuId === m.id && (
                          <div style={{ position: 'absolute', top: '100%', [isMe ? 'right' : 'left']: 0, zIndex: 100, background: '#1a1a1a', border: '1px solid #D4AF37', borderRadius: '8px', marginTop: '5px', display: 'flex', flexDirection: 'column', width: 'max-content' }}>
  ...
  return (
    <div style={{ width: '100%', height: '100dvh', display: 'flex', flexDirection: 'column', background: '#000', color: '#fff', overflow: 'hidden', fontFamily: 'serif' }}>
      <header style={{ padding: 'env(safe-area-inset-top) 15px 10px', background: '#800020', borderBottom: '1px solid #D4AF37', textAlign: 'center', zIndex: 10 }}>
        <h1 style={{ fontSize: '1.4rem', fontStyle: 'italic', fontWeight: 'bold', margin: '10px 0', letterSpacing: '2px', color: '#fff' }}>for VAU <span style={{ fontSize: '0.9rem', color: '#D4AF37' }}>-HOST-</span></h1>
      </header>
          ...
          <div style={{ width: '65px', borderRight: '1px solid #222', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '15px', padding: '10px 0', flexShrink: 0, background: '#050505' }}>
            {sortedGuests.map(g => (
              <div key={g.id} onClick={() => setSelectedGuestId(g.id)} style={{ cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                ...
                <div style={{ fontSize: '0.5rem', color: selectedGuestId === g.id ? '#D4AF37' : '#555', marginTop: '4px' }}>{g.username?.substring(0, 4)}</div>
              </div>
            ))}
          </div>
        )}
      ...
      <footer style={{ padding: `10px 15px calc(10px + env(safe-area-inset-bottom))`, background: '#800020', borderTop: '1px solid #D4AF37', display: 'flex', justifyContent: 'center', gap: '60px', zIndex: 10 }}>
        {['GLOBAL', 'DIRECT'].map(mode => (
          <button key={mode} onClick={() => setViewMode(mode)} style={{ background: 'transparent', color: viewMode === mode ? '#D4AF37' : 'rgba(255,255,255,0.5)', border: 'none', fontSize: '0.8rem', fontWeight: 'bold', borderBottom: viewMode === mode ? '2px solid #D4AF37' : '2px solid transparent', padding: '5px 0' }}>{mode}</button>
        ))}
      </footer>
    </div>
  );
...
// ここでビルドエラーを回避しつつ、default exportを行う
export default dynamic(() => Promise.resolve(AdminComponent), { ssr: false });
