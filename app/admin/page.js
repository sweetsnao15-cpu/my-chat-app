...
export default function GuestPage() {
  ...
  useEffect(() => {
    if (!user) return;
    const channel = supabase.channel(`guest_${user.id}`)
  ...
  const handleSend = async (content, isImage = false) => {
    ...
    await supabase.from('messages').insert({
      content, user_id: user.id, receiver_id: ADMIN_ID, is_image: isImage, is_read: false
    });
        ...
        {messages.map(m => {
          ...
          return (
            <div key={m.id}>
              ...
              <div onContextMenu={(e) => handleContextMenu(e, m)} style={{ marginBottom: '25px', display: 'flex', flexDirection: 'column', alignItems: isMe ? 'flex-end' : 'flex-start' }}>
  ...
  if (!user) {
    return (
      <div style={{ height: '100dvh', background: '#000', color: '#D4AF37', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: 'serif' }}>
        <h1 style={{ fontStyle: 'italic', marginBottom: '30px' }}>for VAU</h1>
        <button onClick={handleLogin} style={{ background: 'transparent', border: '1px solid #D4AF37', color: '#D4AF37', padding: '10px 40px', borderRadius: '20px', cursor: 'pointer' }}>LOGIN</button>
      </div>
    );
  }
    ...
    <div onClick={() => setContextMenu(null)} style={{ width: '100%', height: '100dvh', display: 'flex', flexDirection: 'column', background: '#000', color: '#fff', overflow: 'hidden', fontFamily: 'serif' }}>
      ...
      <header style={{ padding: '10px 20px', background: '#800000', borderBottom: '1px solid #D4AF37', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <input type="text" value={profile.username} onChange={e => setProfile({ ...profile, username: e.target.value })} onBlur={updateProfile} placeholder="Name" style={{ background: 'transparent', border: 'none', borderBottom: '1px solid #D4AF37', color: '#fff', fontSize: '0.8rem', width: '80px', outline: 'none' }} />
          <button onClick={() => supabase.auth.signOut()} style={{ background: 'transparent', border: 'none', color: '#D4AF37', fontSize: '0.7rem' }}>EXIT</button>
        </div>
      </header>
      ...
      <div style={{ padding: '10px 15px', background: '#800000', borderTop: '1px solid #D4AF37', flexShrink: 0 }}>
        <div style={{ maxWidth: '600px', margin: '0 auto', display: 'flex', gap: '8px', alignItems: 'flex-end' }}>
          ...
          <button onClick={() => handleSend(inputText)} style={{ background: '#000', color: '#D4AF37', padding: '8px 18px', borderRadius: '18px', fontWeight: 'bold', border: '1px solid #D4AF37', fontSize: '13px', fontFamily: 'serif' }}>SEND</button>
        </div>
      </div>
    </div>
  );
